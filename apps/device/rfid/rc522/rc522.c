...
// SDA (CS)		    PA4	                SPI1_NSS	Chip select for SPI
// SCK	            PA5	                SPI1_SCK	Serial Clock for SPI
// MOSI	            PA7                 SPI1_MOSI	Master In Slave Out for SPI
// MISO	            unused	            SPI1_MISO	Master Out Slave In for SPI
// IRQ	            unused              Irq
// GND	            GND	                Ground
...
void MFRC522_Port_Init()
{
    // RC522 reset pin must high lever
    MX_SPI1_Init();
}

// SPI speed 18MBits/s      CS is soft control
static uint8_t RC522_SPI_SendByte(uint8_t data)
{
    uint8_t readValue;
    HAL_SPI_TransmitReceive(&hspi1, &data, &readValue, 1, 10);
    return readValue;
...
static void RC522_SPI_WriteReg(uint8_t address, uint8_t value)
{
    cs_reset();
    RC522_SPI_SendByte(address);
    RC522_SPI_SendByte(value);
    cs_set();
}

static uint8_t SPI1_ReadReg(uint8_t address)
{
    uint8_t val;
